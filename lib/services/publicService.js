...
export async function getHero(pageKey) {
  const [rows] = await query(
    'SELECT * FROM hero_sections WHERE active = 1 AND page_key = ? ORDER BY sort_order ASC, id ASC LIMIT 1',
    [pageKey],
  );
  return rows[0] || null;
}
...
export async function getProjectBySlug(slug) {
  ...
  const [[imageRows], [technologyRows]] = await Promise.all([
    query('SELECT * FROM project_images WHERE project_id = ? ORDER BY sort_order ASC, id ASC', [project.id]),
    query(
      `SELECT t.*
